...
namespace XiheFramework {
    public class BlackboardModule : GameModule {
        ...
        public BaseSaveData CreateSaveData() {
            BaseSaveData saoriSaveData = new BaseSaveData {
                saveData = m_SaveDataBlackboard,
            };
            return saoriSaveData;
        }

        public void LoadSaveData(BaseSaveData saoriSaveData) {
            m_SaveDataBlackboard.Clear();
            //给blackboard赋值
            foreach (var variable in saoriSaveData.saveData) {
                m_SaveDataBlackboard.Add(variable.Key, variable.Value);
            }

            //加载对应场景
            //Game.Scene.LoadScene(Game.Blackboard.GetData<string>(XiheKeywords.CurrentSceneName));
        }
